...
SerializableState = SerializableState> extends Component<TBehaviour> {
  ...
  preStart() {
    if (this.enableDebugLogging) {
      console.log("Behaviour Awake - Entity", this.entity.id);
    }
  ...
  start() {
    if (this.enableDebugLogging) {
      console.log("Behaviour Start - Entity", this.entity.id);
    }
  ...
  //------------------------------------------------------------------------------
  // In most cases, you can use class.getComponents(type)[0], but it won't work for interfaces, this will.
