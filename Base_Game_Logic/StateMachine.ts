...
// CREATOR LEVEL: <<Intermediate-Advanced>>
...
{
  static time : Map<string, number> = new Map<string, number>();
}
...
class State{
  ...
  export class StateMachine{
    ...
    private stateConfigName;
    public currentState:State | undefined;
    public isActive:boolean;
    public timer:number;

    constructor(stateArray:string[], configArray:StateConfigRecord[], stateConfigName:string, enableLogging = false) {
      ...
      this.timer = -1;
      StateTimer.time.set(this.stateConfigName, -1);
    ...
    public changeState(stateName:string) {
      ...
      const nextState = this.stateMap.get(stateName);
      ...
      if (nextState != undefined) {
        ...
        // Reset timer
        this.timer = 0;
        StateTimer.time.set(this.stateConfigName, this.timer);
    ...
    public update(deltaTime:number) {
      ...
      for (let i = 0; i < this.currentState.nextStateEdges.length; i++) {
    ...
    private config(configArray:StateConfigRecord[]) {
      configArray.forEach( config => {
        let state = this.stateMap.get(config.name);
    ...
    private chooseNextState(possibleStates:[string, number][]){
      ...
      totalOdds = Math.random() * totalOdds;
